...
class IdeaInfo extends Component {
    ...
    renderUsers = ({ item }) => (
        <Image source={{ uri: item.photo ? item.photo : 'https://lifehacker.ru/wp-content/themes/lifehacker/static/img/no_avatar.jpg' }} style={{ height: 50, width: 50, borderRadius: 25, marginLeft: 5, marginRight: 5 }} />
    );

    renderComments = ({ item }) => (
        <View style={{ paddingBottom: 5, paddingTop: 5, flexDirection: "row", borderColor: "grey", borderBottomWidth: 0.5, width: "100%" }}>
            <Image style={{ height: 50, width: 50, borderRadius: 25, overflow: "hidden" }} source={{ uri: item.photo ? item.photo : 'https://lifehacker.ru/wp-content/themes/lifehacker/static/img/no_avatar.jpg' }} />
    ...
    users = () => {
        ...
        if (params.team) {
            ...
            return <Text style={{ padding: 10, width: "100%",  fontSize: 17 }}>Нет участников</Text>
        }
    }
    ...
    coments = () => {
        ...
        if (params.comments) {
            ...
            return <Text style={{ padding: 10, width: "100%", fontSize: 17 }}>Нет коментариев</Text>
        }
    }
}
